...
cmd = subprocess.run('uname -o', shell=True, capture_output=True, text=True)
PLATFORM = cmd.stdout.lower()
...
class YTSThread(BotThread):
    ...
    def _getLastMovies(self):
        ...
        try:
            ...
            res = requests.get(self._url, timeout=(
                5, 30), cookies=self._cookies)
            ...
            for box in boxes:
                ...
                released = box.find(
                    'div', class_='browse-movie-year').text.strip()
                cover = box.find('img').attrs.get('src').strip()
                link = box.find(
                    'a', class_='browse-movie-title').attrs.get('href').strip()
                try:
                    availableIn = [child.text.strip() for child in box.find(
                        'div', class_='browse-movie-tags').children if child.name == 'a']
            ...
            if posts:
                ...
                for post in posts:
                    ...
                    post['downloaded_cover'] = download(
                        post['cover'], rename_to=f"{self.getName()}_{common.cleanUp(post['title'])}")
    ...
    def _notifyMe(self, posts: dict):
        for post in posts:
            ...
            if PLATFORM == 'android':
                os.system(
                    f'termux-notification -t "{title}" -c "{message}" --sound --image-path {cover} --vibrate 1000')
